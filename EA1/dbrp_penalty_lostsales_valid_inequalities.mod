...
subject to{
    ...
    /*Valid inequalities*/
	forall(t in time, m in machines)
	  	sum(tt in 1..t)(sum(n in nodes)(visit[n][tt]*machineLocation[tt,m,n]))>=
	  		((sum(tt in 1..t) fuelConsumption[m][tt])-initialTankLevel[m]-(sum(tt in 0..t) stockout[m][tt]))/minl(bowserCapacity,tankCapacity[m]);
	
	forall(m in machines, i in time, j in time)
	  /*minl*/(/*1,*/sum(n in nodes, k in i..j)(visit[n][k]*machineLocation[k,m,n]))*(sum(k in 1..T) fuelConsumption[m][k])
	  		>= sum(k in i..j)qty[m][k];
	
	forall(t in time, j in 1..t, m in machines)
	    initialTankLevel[m] + 
		sum(tt in 1..j-1) qty[m][tt] +
		sum(tt in 0..t) stockout[m][tt] - 
		sum(tt in 1..t) fuelConsumption[m][tt]>=
		-(sum(k in 1..t) fuelConsumption[m][k])*/*minl*/(/*1,*/sum(n in nodes, k in j..t)(visit[n][k]*machineLocation[k,m,n]));
}