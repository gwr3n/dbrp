...
subject to{
	...
	forall(n1 in nodes, n2 in nodes, t in 2..T) transit[t-1][n1][n2]>=visit[n1][t-1]+visit[n2][t]-1;
	...
	/*CUTS*/
	//if a machine has enough fuel, you should not refuel it (preprocessing)
	//forall(m in machines, t in time) initialTankLevel[m] - sum(tt in 1..T) fuelConsumption[m][tt] >= 0 => qty[m][t] == 0;
	
	/*Safety constraints*/
	//forbidden to remain stationary at any node except node 1
	//forall(n in 2..N, t in 2..T) transit[t-1][n][n] == 0;
}